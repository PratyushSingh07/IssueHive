...
import com.example.findissues.FindIssuesApp
...
object Toaster {
    ...
    fun show(view: View?, text: String?, duration: Int = Snackbar.LENGTH_LONG) {
        val imm = FindIssuesApp.context?.getSystemService(Context.INPUT_METHOD_SERVICE) as? InputMethodManager
    ...
    fun show(view: View, res: Int, duration: Int) {
        show(view, FindIssuesApp.context?.resources?.getString(res), duration)
    }
    ...
    fun show(view: View?, res: Int) {
        show(view, FindIssuesApp.context?.resources?.getString(res))
    }
}
